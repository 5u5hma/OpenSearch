...
// ── TEST ONLY: Deliberate leak for profiling validation ─────────────────────
// Each call allocates 1MB and never frees it. Remove after testing.
static LEAK_STORAGE: std::sync::Mutex<Vec<Vec<u8>>> = std::sync::Mutex::new(Vec::new());

/// FFI: Deliberately leaks 1MB per call. For testing heap profiling only.
#[no_mangle]
pub extern "C" fn native_jemalloc_test_leak(count: i64) -> i64 {
    ffm_wrap("native_jemalloc_test_leak", || {
        let mut storage = LEAK_STORAGE.lock().unwrap();
        for _ in 0..count {
            // Allocate 1MB and never free
            storage.push(vec![42u8; 1024 * 1024]);
        }
