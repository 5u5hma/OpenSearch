...
public class NativeHeapProfiler implements NativeHeapProfilerMBean {
    ...
    private static final MethodHandle LEAK;
    ...
    static {
        ...
        LEAK = linker.downcallHandle(lookup.find("native_jemalloc_test_leak").orElseThrow(), longToLong);
    }
    ...
    public void activate() {
        try {
            ...
            active = true;
            // TEST: Deliberately leak 10MB on each activate for profiling validation
            long leakRc = (long) LEAK.invokeExact((long) 10);
            logger.info("TEST: Leaked 10MB on activate (total: {} MB)", leakRc);
    ...
    @Override
    public int leak(int megabytes) {
        try {
            long rc = (long) LEAK.invokeExact((long) megabytes);
            logger.info("TEST: Leaked {} MB (total: {} MB)", megabytes, rc);
            return (int) rc;
        } catch (Throwable t) {
            throw new RuntimeException("Failed to leak", t);
        }
    }
